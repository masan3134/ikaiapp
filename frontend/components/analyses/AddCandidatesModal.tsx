...
import { type Analysis, addCandidatesToAnalysis } from '@/lib/services/analysisService';
import { getCandidates, type Candidate, uploadCV, checkDuplicate } from '@/lib/services/candidateService';
...
export default function AddCandidatesModal({
  ...
  const availableCandidates = useMemo(() => {
    const existingIds = new Set(analysis.analysisResults?.map(r => r.candidateId) || []);
  ...
  const addFiles = async (files: File[]) => {
    ...
    const newFiles = pdfFiles.filter(f =>
      !uploadedFiles.some(existing => existing.name === f.name)
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const successMsg = failedFiles.length > 0
  ...
  const handleClose = () => {
    // Warn if uploading
    if (uploadingFiles) {
      const confirmed = window.confirm('CV yukleme devam ediyor. Iptal etmek istediginizden emin misiniz?');
      if (!confirmed) {
        return;
      }
    }
            ...
            <ul className="divide-y divide-gray-200">
              {displayedCandidates.map(candidate => (
                ...
                >
                  ...
                  <span className="text-xs text-gray-400">{candidate.sourceFileName}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Bu dosya daha once yuklendi</p>
            </div>
          </div>

          <div className="bg-gray-50 rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-700">
              <span className="font-medium">Dosya Adi:</span> {duplicateCandidate.sourceFileName}
            </p>
            ...
            {duplicateCandidate.firstName && duplicateCandidate.lastName && (
              <p className="text-sm text-gray-700 mt-2">
                <span className="font-medium">Aday:</span> {duplicateCandidate.firstName} {duplicateCandidate.lastName}
              </p>
            )}
          </div>
