  ...
  async function handleSendTests() {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          reused ? "♻️ Mevcut test kullanılıyor" : "🆕 Yeni test oluşturuldu"
        );
      }
      ...
      for (const result of selectedList) {
        try {
          ...
          if (process.env.NODE_ENV === "development") {
            console.error(
              `Failed to send test to ${result.candidate.email}:`,
              error
            );
          }
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-purple-600 text-white p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-purple-100 text-sm mt-1">{jobTitle}</p>
          </div>
          <button
            onClick={onClose}
            disabled={sending}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mt-6 bg-blue-50 border-2 border-blue-300 rounded-lg p-4">
            ...
            <ul className="space-y-1 text-sm text-gray-800">
              <li>
                • Bu iş ilanı için <strong>TEK BİR TEST</strong> oluşturulur ve
                tüm adaylara gönderilir
              </li>
              <li>
                • Test sorularını yapay zeka ilanı detaylı analiz ederek
                oluşturur
              </li>
              <li>
                • 10 soru (Teknik, Senaryosal, Deneyim) - 30 dakika - 3 deneme
                hakkı
              </li>
