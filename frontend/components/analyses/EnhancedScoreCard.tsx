...
export default function EnhancedScoreCard({
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={`rounded-xl p-6 border-2 ${getScoreBgColor(scores.finalCompatibilityScore)}`}
      >
        <div className="text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            <Award
              className={`w-6 h-6 ${getScoreColor(scores.finalCompatibilityScore)}`}
            />
            <span className="text-sm font-medium text-gray-600">
              Nihai Uygunluk Skoru
            </span>
          </div>
          <div
            className={`text-5xl font-bold mb-2 ${getScoreColor(scores.finalCompatibilityScore)}`}
          >
      ...
      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
        <p className="text-xs text-gray-600 text-center">
          <span className="font-semibold">Not:</span> V7.1 sisteminde tüm
          skorlar 0-100 aralığındadır ve pozisyona göre ağırlıklandırılır.
        </p>
      </div>
    </div>
  );
