...
const AcceptanceForm: React.FC<AcceptanceFormProps> = ({
  ...
  const handleReject = async () => {
    const reason = prompt(
      "Reddetme sebebinizi belirtebilir misiniz? (Opsiyonel)"
    );
    ...
    return (
      <div className="bg-red-50 border-l-4 border-red-500 p-4 my-8">
        ...
        <p className="text-red-700">
          Lütfen daha fazla bilgi için işe alım uzmanıyla iletişime geçin.
        </p>
      </div>
    );
  }
  ...
  if (isResponded) {
    return (
      ...
      >
        ...
        <p
          className={
            offer.status === "accepted" ? "text-green-700" : "text-gray-700"
          }
        >
    ...
    <div>
      <div className="text-center my-10">
        <p className="text-gray-600 mb-4">
          Teklifi değerlendirmek ve yanıtlamak için aşağıdaki butonları
          kullanabilirsiniz.
        </p>
      ...
      <p className="text-center text-sm text-gray-500 mt-6">
        📅 Bu teklif {new Date(offer.expiresAt).toLocaleDateString("tr-TR")}{" "}
