...
export default function NotificationItem({
  ...
  const { id, type, title, message, read, createdAt, user, organization } = notification;
    ...
    >
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          {/* Title */}
          <p className={`text-sm font-medium ${!read ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
            {title}
          </p>

          {/* Message */}
          <p className={`mt-1 text-sm ${!read ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'} ${compact ? 'line-clamp-1' : 'line-clamp-2'}`}>
            {message}
          </p>
