...
export default function UsageWidget() {
  ...
  if (loading) {
    return (
      <Card
        ...
        subtitle="Aylık kullanım limitleriniz"
      >
      ...
      >
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-600">
            Kullanım bilgileri yüklenemedi
          </p>
          <button
            onClick={() => execute()}
  ...
  const hasCriticalWarnings = usage.warnings.some(
      ...
      {usage.plan === "PRO" || usage.plan === "ENTERPRISE" ? (
        <div className="mt-4 flex items-center gap-2 text-green-600 bg-green-50 rounded-lg p-3">
          ...
          <span className="text-sm font-medium">
            {usage.plan === "PRO" ? "PRO Plan aktif" : "ENTERPRISE Plan aktif"}{" "}
            - Sınırsız kullanım
          </span>
        </div>
      ) : null}
