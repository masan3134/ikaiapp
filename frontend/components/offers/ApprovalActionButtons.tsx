...
const ApprovalActionButtons: React.FC<ApprovalActionButtonsProps> = ({ offer, onActionComplete }) => {
  const { user } = useAuth();
  ...
  if (!offer || !user || !['ADMIN', 'MANAGER'].includes(user.role) || offer.approvalStatus !== 'pending') {
    return null;
  }
  ...
  return (
    <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg shadow-md p-6 border-l-4 border-purple-500">
      <h4 className="text-lg font-bold text-gray-900 mb-2">⚡ Yönetici Aksiyonları</h4>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h5 className="text-xl font-bold text-gray-900 mb-2">Teklifi Reddet</h5>
            <p className="text-gray-600 mb-4">Lütfen bu teklifi reddetme nedeninizi belirtin.</p>
            <textarea
              rows={4}
              value={reason}
