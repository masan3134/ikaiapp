          ...
          {templates.map((template) => (
            <div key={template.id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
              ...
              <div className="space-y-2 text-sm text-gray-700 mb-4">
                <p><strong>Pozisyon:</strong> {template.position}</p>
                <p><strong>Departman:</strong> {template.department}</p>
                <p><strong>Maaş:</strong> ₺{template.salaryMin.toLocaleString()} - ₺{template.salaryMax.toLocaleString()}</p>
              </div>

              <div className="flex gap-2">
                <button
                  onClick={() => router.push(`/offers/templates/${template.id}`)}
