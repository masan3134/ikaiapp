...
function WizardPage() {
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      const { isNewJobPosting, newJobPostingData, setJobPosting } =
        ...
        try {
          ...
          const uploadPromises = uploadedFiles.map(async (file, index) => {
            try {
              ...
              // apiClient auto-handles success
              const data = response.data;
  ...
  const handleStartAnalysis = async () => {
    ...
    if (
      !confirm(
        `${totalCandidates} aday için analiz başlatılacak. Devam etmek istiyor musunuz?`
      )
    ) {
      return;
    }
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  if (
                    confirm(
                      "Sihirbazdan çıkmak istediğinize emin misiniz? Tüm ilerleme kaybedilecek."
                    )
                  ) {
                    resetWizard();
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-700 mt-1">
                    {uploadProgress.completed} / {uploadProgress.total} dosya
                    tamamlandı
                    {uploadProgress.failed > 0 &&
                      ` (${uploadProgress.failed} başarısız)`}
                  </p>
                </div>
