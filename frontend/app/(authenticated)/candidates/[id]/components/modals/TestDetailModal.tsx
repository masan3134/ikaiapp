...
import { Mail, User, Calendar, CheckCircle, XCircle, Clock } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b-2 border-gray-200 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Test Sonucu Detayı</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-300 rounded-xl p-6">
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div className="text-5xl font-bold text-purple-600">{test.score}</div>
                <div className="text-sm font-bold text-gray-700">/ 100</div>
              </div>
              <div className="flex-1 space-y-2">
                <div>
                  <span className="text-sm font-bold text-gray-900">Doğru Cevap:</span>
                  <span className="ml-2 text-base font-bold text-green-700">
                    {test.correctCount} / 10
                  </span>
                </div>
                <div>
                  <span className="text-sm font-bold text-gray-900">Yanlış Cevap:</span>
                  <span className="ml-2 text-base font-bold text-red-700">
                    {10 - test.correctCount} / 10
                  </span>
                </div>
                <div>
                  <span className="text-sm font-bold text-gray-900">Deneme:</span>
          ...
          <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-5">
            <h3 className="text-lg font-bold text-gray-900 mb-3">Aday Bilgileri</h3>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Mail className="w-4 h-4 text-blue-600" />
                <span className="text-sm font-bold text-gray-900">Email:</span>
                <span className="text-sm text-gray-800 font-medium">{test.candidateEmail}</span>
              </div>
              {test.candidateName && (
                <div className="flex items-center gap-2">
                  <User className="w-4 h-4 text-blue-600" />
                  <span className="text-sm font-bold text-gray-900">Ad Soyad:</span>
                  <span className="text-sm text-gray-800 font-medium">{test.candidateName}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-gray-50 border-2 border-gray-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-bold text-gray-900">Test Başlangıç</span>
              </div>
            ...
            <div className="bg-gray-50 border-2 border-gray-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-bold text-gray-900">Test Bitiş</span>
              </div>
              <p className="text-base text-gray-800 font-medium">
          ...
          {test.metadata && (
            <div className="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-5">
              <h3 className="text-lg font-bold text-gray-900 mb-3">🛡️ Güvenlik İstatistikleri</h3>
              <div className="grid md:grid-cols-2 gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">Tab Değiştirme:</span>
                  <span
                    className={`text-sm font-bold ${
                      test.metadata.tabSwitchCount > 5 ? 'text-red-600' : 'text-green-600'
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">Kopyalama Denemesi:</span>
                  <span
                    className={`text-sm font-bold ${
                      test.metadata.copyAttempts > 0 ? 'text-red-600' : 'text-green-600'
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">Ekran Görüntüsü:</span>
                  <span
                    className={`text-sm font-bold ${
                      test.metadata.screenshotAttempts > 0 ? 'text-red-600' : 'text-green-600'
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">Yapıştırma Denemesi:</span>
                  <span
                    className={`text-sm font-bold ${
                      test.metadata.pasteAttempts > 0 ? 'text-red-600' : 'text-green-600'
                    }`}
                  >
                ...
                {test.metadata.autoSubmit && (
                  <div className="md:col-span-2 flex items-center gap-2 text-orange-700">
                    <span className="text-sm font-bold">⚠️ Otomatik Gönderim (Süre Doldu)</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {test.test?.questions && test.answers && (
            <div className="bg-gray-50 border-2 border-gray-300 rounded-lg p-5">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Soru Detayları</h3>
              <div className="space-y-4">
                {test.test.questions.map((question, qIdx) => {
                  const userAnswer = test.answers?.find((a) => a.questionId === question.id);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-gray-900">Soru {qIdx + 1}:</span>
                          <span
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-300">
                        <div className="flex items-center justify-between text-sm">
                          <div>
                            <span className="font-bold text-gray-900">Adayın Cevabı: </span>
                            {isAnswered ? (
                              <span
                                className={`font-bold ${
                              ...
                              <span className="font-bold text-gray-600">Cevapsız</span>
                            )}
                          </div>
                          <div>
                            <span className="font-bold text-gray-900">Doğru Cevap: </span>
          ...
          <div className="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-900">Son Geçerlilik:</span>
