...
export async function submitTest(token: string, data: {
  candidateEmail: string;
  candidateName?: string;
  answers: any[];
  startedAt: string;
}) {
  const response = await apiClient.post(`/api/v1/tests/public/${token}/submit`, data);
  return response.data;
}
...
export async function generateTest(data: { jobPostingId: string; analysisId?: string }) {
  const response = await apiClient.post(`/api/v1/tests/generate`, data);
  return response.data;
}

// Send test email
export async function sendTestEmail(testId: string, data: { recipientEmail: string; recipientName?: string }) {
  const response = await apiClient.post(`/api/v1/tests/${testId}/send-email`, data);
  return response.data;
}
